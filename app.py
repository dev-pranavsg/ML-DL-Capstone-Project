...
@st.cache_data
def load_real_data():
    if os.path.exists("demo_windows.npy"):
        return np.load("demo_windows.npy")
    return None
...
if st.button("Generate & Test Random Window"):
    if real_windows is not None:
        random_idx = np.random.randint(len(real_windows))
        selected_window = real_windows[random_idx]
        test_window = selected_window.tolist()
    else:
